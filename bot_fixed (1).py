...
def build_html(rows: List[Dict[str, str]], settings: Dict[str, Any], user_id: int) -> str:
    ...
    wm_html = ""
    if use_image_wm:
        # Use base64 data URI so WeasyPrint always finds the image regardless
        # of base_url or working-directory issues.
        try:
            _wm_mime = "image/png" if u_wm.suffix.lower() == ".png" else "image/jpeg"
            _wm_b64 = base64.b64encode(u_wm.read_bytes()).decode()
            _wm_src = f"data:{_wm_mime};base64,{_wm_b64}"
        except Exception:
            _wm_src = f"file://{u_wm.resolve()}"
        wm_html = (
            f"<div class='watermark-img'>"
            f"<img src='{_wm_src}' alt='' style='opacity:{opacity};'/>"
            f"</div>"
        )
    ...
    if settings.get("logo_enabled"):
        if u_logo.exists():
            try:
                _logo_mime = "image/png" if u_logo.suffix.lower() == ".png" else "image/jpeg"
                _logo_b64 = base64.b64encode(u_logo.read_bytes()).decode()
                _logo_src = f"data:{_logo_mime};base64,{_logo_b64}"
            except Exception:
                _logo_src = f"file://{u_logo.resolve()}"
            logo_html = (
                "<td class='logo'>"
                f"<div class='logo-circle'><img src='{_logo_src}' alt='logo'/></div>"
                "</td>"
            )
        else:
...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  .watermark-img {{ position: fixed; top: -14mm; left: -12mm; right: -12mm; bottom: -18mm; display: flex; align-items: center; justify-content: center; pointer-events: none; z-index: 0; overflow: hidden; }}
  .watermark-img img {{ display: block; width: 130mm; max-width: 65%; height: auto; max-height: 55%; object-fit: contain; margin: auto; }}
...
async def cmd_setjoinmsg(update: Update, context: ContextTypes.DEFAULT_TYPE, args: str) -> None:
    ...
    if not text:
        await msg.reply_text(
            ...
            "• <code>{user_name}</code> / <code>{fullname}</code> — full display name\n"
            "• <code>{firstname}</code> — first name (like Telegram mention)\n"
            "• <code>{lastname}</code> — last name\n"
            "• <code>{username}</code> — @handle (or name fallback)\n"
            "  (also works with <code>%firstname%</code> style)\n\n"
            "<b>Inline links</b>\n"
...
def _watermark_overlay_for_size(uid: int, settings: Dict[str, Any], width_pt: float, height_pt: float) -> Optional[bytes]:
    ...
    if use_image:
        _p = wm_path(uid)
        try:
            _mime = "image/png" if _p.suffix.lower() == ".png" else "image/jpeg"
            _b64 = base64.b64encode(_p.read_bytes()).decode()
            img_uri = f"data:{_mime};base64,{_b64}"
        except Exception:
            img_uri = f"file://{_p.resolve()}"
        inner = (
            f"<div class='wm'>"
    ...
    html_doc = f"""<!doctype html><html><head><meta charset='utf-8'><style>
        ...
        .wm {{ position: fixed; top: 0; left: 0; right: 0; bottom: 0; display: flex; align-items: center; justify-content: center; overflow: hidden; }}
        .wm img {{ display: block; width: 60%; max-width: 65%; height: auto; max-height: 55%; object-fit: contain; margin: auto; }}
...
def _apply_watermark_to_pdf(uid: int, settings: Dict[str, Any], body_pdf: bytes) -> bytes:
    ...
    placed ON TOP of the original page content (over=True) with the configured
    opacity so it remains visible regardless of the page's background color.
    Using over=False hides the watermark behind opaque page backgrounds.
    """
    try:
    ...
    for page in reader.pages:
        try:
            ...
            if wm_reader is not None and len(wm_reader.pages) > 0:
                ...
                # over=True: watermark drawn ON TOP — always visible even on white-background pages.
                # The opacity in the watermark XObject handles translucency so content shows through.
